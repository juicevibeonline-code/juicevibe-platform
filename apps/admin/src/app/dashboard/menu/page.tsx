    ...
    <div className="space-y-6">
      ...
      {itemsLoading || categoriesLoading ? (
        <div className="flex flex-col items-center justify-center py-24 font-mono text-xs text-muted-foreground uppercase tracking-widest gap-3">
          ...
          <span>Compiling catalog menu parameters...</span>
        </div>
      ) : menuItems.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
          {menuItems.map((item) => {
            const isDeleting = deleteItemMutation.isPending && (deleteItemMutation.variables as string) === item.id;
            return (
              <div 
                key={item.id}
              ...
              >
                ...
                <div className="flex items-center gap-2 border-t border-border/40 pt-4 mt-5">
                  ...
                  <button
                    ...
                    disabled={isDeleting}
                    className="p-1.5 border border-border hover:border-pink/40 hover:bg-pink/5 text-muted-foreground hover:text-pink rounded cursor-pointer transition-colors disabled:opacity-50"
                  >
                    {isDeleting ? (
