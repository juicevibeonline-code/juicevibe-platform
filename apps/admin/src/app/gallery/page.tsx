    ...
    <div className="space-y-6 max-w-7xl mx-auto px-2 animate-fade-in pb-12">
      <div className="relative p-8 rounded-[2rem] glass-panel overflow-hidden mb-8">
        ...
        <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            <h1 className="text-3xl font-black text-gray-800 tracking-tight">Gallery Management</h1>
            <p className="text-gray-500 font-medium mt-2">Manage your image gallery and assets</p>
          </div>
        ...
        {categories.map((cat) => (
          <button
            ...
            className={`px-5 py-2 rounded-xl text-sm font-bold capitalize transition-all duration-300 hover:-translate-y-0.5 ${
              activeCategory === cat ? "bg-gradient-to-r from-primary to-primary-dark text-white shadow-[0_4px_15px_rgba(34,197,94,0.3)]" : "bg-white/60 text-gray-600 hover:bg-white hover:text-gray-900 border border-white/80 shadow-sm"
            }`}
          >
            {cat}
          </button>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 px-2 mt-4">
        {images
          ...
          .map((img) => (
            <div key={img.id} className="group relative rounded-[2rem] overflow-hidden bg-white/40 border border-white/80 aspect-square shadow-sm hover:shadow-xl hover:-translate-y-2 transition-all duration-500">
              ...
              <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center backdrop-blur-[2px]">
                <button className="p-3 bg-pink/90 rounded-xl hover:scale-110 hover:bg-pink transition-all duration-300 shadow-lg">
