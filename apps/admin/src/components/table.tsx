...
export function Table<T extends Record<string, any>>({
  ...
  return (
    <div className="bg-card border border-border rounded-lg shadow-sm flex flex-col relative overflow-hidden">
      {searchable && (
        <div className="p-4 border-b border-border bg-background/50 shrink-0 flex items-center justify-between gap-4 flex-wrap">
          <div className="relative w-full max-w-sm group">
            ...
            <input
              ...
              className="w-full pl-9 pr-10 py-1.5 rounded-lg border border-border bg-card text-xs focus:outline-none focus:border-primary/50 transition-colors text-foreground font-medium"
            />
            ...
            <TableRow className="hover:bg-transparent">
              {columns.map((col) => (
                <TableHead 
                  key={col.key} 
                  className={cn(
                    "px-5 py-3 text-[10px] font-bold text-muted-foreground uppercase tracking-wider",
                    col.sortable && "cursor-pointer hover:bg-background transition-colors"
                  )}
          ...
          <TableBody className="divide-y divide-border">
            {isLoading ? (
              ...
              data.map((item, i) => (
                <TableRow 
                  key={item.id || i}
                  className="hover:bg-background/50 border-b border-border last:border-0 transition-colors"
                >
                  {columns.map((col) => (
                    <TableCell key={col.key} className="px-5 py-3 text-xs font-semibold text-foreground">
      ...
      {totalPages > 1 && (
        <div className="flex items-center justify-between p-4 border-t border-border bg-background/50 shrink-0">
