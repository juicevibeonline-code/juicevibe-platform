        ...
        <div className="p-4 border-b border-border bg-slate-50/50 dark:bg-zinc-900/10 shrink-0 flex items-center justify-between gap-4 flex-wrap">
          <div className="relative w-full max-w-sm group">
            ...
            <input
              ...
              className="w-full pl-8 pr-10 py-1.5 rounded-lg border border-border/80 bg-card text-xs focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary/20 transition-all text-foreground font-semibold placeholder:text-muted shadow-sm"
            />
            <kbd className="absolute right-2.5 top-1/2 -translate-y-1/2 pointer-events-none hidden sm:inline-flex h-5 select-none items-center gap-0.5 rounded border border-border/60 bg-muted px-1.5 font-mono text-[9px] font-bold text-muted-foreground shadow-sm">
              <span>/</span>
            </kbd>
          </div>
          {data.length > 0 && (
          ...
          <TableBody className="divide-y divide-border/60">
            {isLoading ? (
              ...
              data.map((item, i) => (
                ...
                >
                  {columns.map((col) => (
                    <TableCell key={col.key} className="px-6 py-4 text-xs font-semibold text-foreground font-sans">
                      {col.render ? (
