    ...
    <div className="bg-transparent h-full flex flex-col">
      <div className="flex items-start justify-between mb-8">
        <div>
          ...
          <p className="text-sm font-medium text-muted mt-1">Weekly revenue performance</p>
        </div>
      ...
      <div className="flex-1 flex items-end gap-3 h-[250px] mt-auto">
        {data.map((value, i) => (
          <div key={i} className="flex-1 flex flex-col items-center gap-3 h-full justify-end group">
            <span className="text-xs font-bold text-muted opacity-0 group-hover:opacity-100 transition-opacity -translate-y-2 group-hover:translate-y-0 duration-300">{value}%</span>
            <div
            ...
            />
            <span className="text-xs font-semibold text-muted group-hover:text-primary-dark transition-colors">{days[i]}</span>
          </div>
        ))}
      </div>
    </div>
